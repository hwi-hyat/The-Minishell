...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/17 00:43:46 by siykim            #+#    #+#             */
/*   Updated: 2023/01/17 00:55:46 by siykim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

char	*env_from_str_sub_question(char *str)
{
	char	*str1;
	char	*str2;
	char	*str3;
...
char	*env_from_str_sub(char *str, int end)
{
	char	*str2;
	char	*str3;
	...
	if (str[++end] != '_' && !(str[end] >= 'A' && str[end] <= 'Z')
		&& !(str[end] >= 'a' && str[end] <= 'z'))
		return (env_from_str_sub_question(str));
...
char	*env_from_str(char *str)
{
	...
	while (split_str[++index] != NULL && ret != NULL)
	{
		...
		if (str_tmp2[0] == '$')
			str_tmp2 = env_from_str_sub(str_tmp2, 0);
		str_tmp = ret;
